...
kleiner_run1_lc <- read.csv("data/kleiner_data/Run1_C4_832ng_oligo_lc-retention-times.csv")
...
broberg_mt_lc_p <- broberg_mt_lc %>% ggplot(aes(rts)) + geom_histogram() + 
  theme_bw() + ylab('') + xlab('Predicted Retention Time (minutes)') + ggtitle('D) Diseased Oak Tree Metatranscriptome') +
...
broberg_mg_oak_lc_p <- broberg_mg_oak_lc %>% ggplot(aes(rts)) + geom_histogram() + 
  theme_bw() + ylab('Count') + xlab('Predicted Retention Time (minutes)') + ggtitle('E) Diseased Oak Tree Genome + Metagenome') +
  theme(legend.position = "none", 
        title = element_text(size = 7.5))
...
aylward_lc_p <- aylward_lc %>% ggplot(aes(rts/60)) + geom_histogram() + 
  theme_bw() + ylab('Count') + xlab('Predicted Retention Time (minutes)') + ggtitle('E) Ant Fungus Garden Metagenome') +
  theme(legend.position = "none", 
        title = element_text(size = 7.5))

jpeg("figures/supp_retention_times_plots.jpeg", width=170, height=160, units="mm", res=850)

grid.arrange(kleiner_run4_lc_p, kleiner_run1_lc_p, broberg_mg_lc_p, broberg_mt_lc_p, 
             # broberg_mg_oak_lc_p, 
             aylward_lc_p, nrow = 3)
