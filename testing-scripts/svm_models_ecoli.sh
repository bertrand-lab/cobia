...
cobia database_trypsin -f "$DIR"uniprot-ecoli.no-decoy.fasta -n "$DIR"uniprot-ecoli.no-decoy_trypsin.txt -c no-write
...
RTPredict -in_text "$DIR"uniprot-ecoli.no-decoy_trypsin.txt -out_text:file "$DIR"uniprot-ecoli.no-decoy_rt_oligo.csv -svm_model "$DIR"ecoli_model_oligo.txt -total_gradient_time 10800
