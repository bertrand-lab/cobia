...
mg_inferred <- obs_protein2 %>% ggplot(aes(x = mean_cofrag_score)) + 
  ...
  annotate("text", x = 80, y = Inf, vjust = 3, label = mginferred_n, parse = TRUE, size = 3, colour = 'grey62') +
  annotate("text", x = 80, y = Inf, vjust = 4.5, label = mginferred_n_sub, parse = TRUE, size = 3, colour = 'grey24') +
...
coef_diagram_inferred$dataset <- c('Diseased Oak\nTree Metagenome', 'Diseased Oak\nTree Metagenome', 'Diseased Oak\nTree Metatranscriptome', 'Diseased Oak\nTree Metatranscriptome')

coef_diagram_inferred$dataset <- factor(coef_diagram_inferred$dataset, 
                                        c("Diseased Oak\nTree Metatranscriptome", 
                                          "Diseased Oak\nTree Metagenome"))
# coef_diagram_inferred$dataset <- as.character(coef_diagram_inferred$dataset)
...
coef_graph_inferred <- ggplot(coef_diagram_inferred, aes(x = dataset, y = coef)) + 
  ...
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 60, vjust = 0.5));coef_graph_inferred
